...
function prepareUI() {
  ...
  ui.download.addEventListener("click", downloadPuzzle);
}
//-----------------------------------------------------------------------------
//------------------------------------------------------------------------
...
function makeSaveFileName(src) {
  ...
  ui?.saveas && (ui.saveas.value = nname);
  return nname;
} // makeSaveFileName
...
function loadInitialFile() {
  let defaultImage = "images/img12.jpg";
...
function puzzleSolved() {
  playing = false;
  document.getElementById("forPuzzle").style.transition = "opacity 1s";
  document.getElementById("forPuzzle").style.opacity = 0;

  setTimeout(() => {
    document.getElementById("forPuzzle").style.display = "none";
    ...
    // Auto-open the envelope and show the modal for immediate reveal
    // This makes the modal appear without user needing to click the letter
    try {
      envelope.classList.remove("close");
      envelope.classList.add("open");
      setTimeout(() => {
  ...
  animate = function (tStamp) {
    ...
    switch (state) {
      ...
      case 155:
        ...
        puzzle.imageLoaded = false;
        if (!puzzle.restoredState.src.startsWith("data:")) {
          puzzle.srcImage.crossOrigin = "Anonymous";
        }
      ...
      case 9999: // puzzle solved, do nothing.
        break;
    } // switch(state)
  }; // animate
} // scope for animate
...
function startPuzzleGame() {
  ...
  autoStart = true; // changed to true for automatic start
  puzzle.nbPieces = 12; // set default number of pieces
  loadInitialFile();
  requestAnimationFrame(animate);
}

// Check if forPuzzle is visible before initializing
const forPuzzleDiv = document.getElementById("forPuzzle");
if (forPuzzleDiv && forPuzzleDiv.style.display !== "none") {
  startPuzzleGame();
} else {
  console.log("Puzzle will be initialized when modal is dismissed");
}
